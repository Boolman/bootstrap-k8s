...
do
  ...
  openstack port list -c ID -c 'Fixed IP Addresses' -f value | grep ${ip} | awk '{print $1}' | xargs -r -IPORTID echo openstack port set --allowed-address ${netone} --allowed-address ${nettwo} PORTID
done < <(cat inventory | grep ansible_ssh_host | cut -d "=" -f2 | sort -u)
